...
class LoRa(object):
    def __init__(
        ...
        self.spi = SPI(
            self._spi_channel[0],
            5000000,
            sck=Pin(self._spi_channel[1]),
            mosi=Pin(self._spi_channel[2]),
            miso=Pin(self._spi_channel[3]),
        )
    ...
    def _handle_interrupt(self, channel):
        ...
        if self._mode == MODE_RXCONTINUOUS and (irq_flags & RX_DONE):
            ...
            if packet_len >= 4:
                ...
                if (self._this_address != header_to) and (
                    (header_to != BROADCAST_ADDRESS) or (self._receive_all is False)
                ):
                    return
                ...
                if (
                    ...
                    and not header_flags & FLAGS_ACK
                ):
